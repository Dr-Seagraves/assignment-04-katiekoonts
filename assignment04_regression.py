...
def estimate_regression(df: pd.DataFrame, x_var: str):
    ...
    """
    # Fit simple OLS regression using a formula.
...
def save_regression_summary(model, output_path: Path) -> None:
    ...
    # Write the statsmodels summary to disk.
...
def plot_scatter_with_regression(
    ...
    plot_df = df[[x_var, "ret"]].dropna().copy()
    ax.scatter(
        plot_df[x_var],
        plot_df["ret"],
        alpha=0.4,
    ...
    x_min, x_max = plot_df[x_var].quantile([0.02, 0.98])
    x_vals = np.linspace(x_min, x_max, 200)
    intercept = model.params.get("Intercept", np.nan)
    slope = model.params.get(x_var, np.nan)
    y_vals = intercept + slope * x_vals
    ax.plot(x_vals, y_vals, color="crimson", linewidth=2, label="Fitted line")

    y_min, y_max = plot_df["ret"].quantile([0.02, 0.98])
    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_min, y_max)

    r_squared = model.rsquared
    ax.set_title(f"{title} (R^2 = {r_squared:.3f})")
...
def print_key_results(model, x_var: str) -> None:
    ...
    slope = params.get(x_var, np.nan)

    print(f"Intercept (beta_0): {intercept:.6f}")
    print(f"Slope (beta_1): {slope:.6f}")
    print(f"SE(beta_0): {bse.get('Intercept', np.nan):.6f}")
    print(f"SE(beta_1): {bse.get(x_var, np.nan):.6f}")
    print(f"t(beta_0): {tvals.get('Intercept', np.nan):.3f}")
    print(f"t(beta_1): {tvals.get(x_var, np.nan):.3f}")
    print(f"p(beta_0): {pvals.get('Intercept', np.nan):.4f}")
    print(f"p(beta_1): {pvals.get(x_var, np.nan):.4f}")
    print(f"R^2: {model.rsquared:.4f}")
    print(f"Adj R^2: {model.rsquared_adj:.4f}")
    print(f"N: {int(model.nobs)}")

    sign = "positive" if slope >= 0 else "negative"
    significant = "significant" if pvals.get(x_var, 1.0) < 0.05 else "not significant"
    print(f"Slope is {sign} and {significant} at 5%.")
    print("=" * 60 + "\n")
